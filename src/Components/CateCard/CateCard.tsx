...
function CateCard({ category, selectedValue, onClick }: CateCardProps) {
  const [isSelected, setIsSelected] = useState(false);

  useEffect(() => {
    setIsSelected(selectedValue === category ? true : false);
  });

  return (
    <div
      className={isSelected ? cx("wrapper", "wrapper-selected") : cx("wrapper")}
      onClick={onClick}
    >
      <div className={cx("circle")}>
        <svg
          ...
          strokeWidth="0"
          viewBox="0 0 512 512"
          className={cx("text-white", "group-hover:text-textColor", "text-lg")}
          height="1em"
          width="1em"
      ...
      <p className={cx("cate-name")}>{category}</p>
    </div>
  );
}
